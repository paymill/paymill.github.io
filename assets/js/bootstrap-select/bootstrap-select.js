            ...
            this.$newElement.on('click', 'li a', function(e){
                ...
                if (_this.$element.not(':disabled') && !$(this).parent().hasClass('disabled')){
                    ...
                    if (!_this.multiple) {
                        ...
                        //If has class changehighlight
                        if (_this.$element.hasClass("changehighlight")) {

                            function select_highlights() {
                                return $('div[class*="highlight-"]').not('.highlight-none');
                            }

                            var code_highlights = select_highlights();
                            var select_highlights = select_highlights();

                            var element = $(this);
                            var language = _this.$element.find('option').eq(clickedIndex).attr("rel")

                                $.cookie('api-v2_docs_language', language, {
                                path: '/'
                            });
                            code_highlights = select_highlights;
                            code_highlights.hide();
                            element.addClass('active');
                            code_highlights.filter('.highlight-' + language).show();
                        }
                    } 
